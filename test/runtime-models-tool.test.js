...
const { createRinBuiltinTools } = require('../dist/runtime.js')

function createTools(modelRegistry, overrides = {}) {
  return createRinBuiltinTools({
...
test('rin_subagent surfaces quota failures clearly', async () => {
  ...
  assert.equal(result.details.errorKind, 'quota_or_rate_limit')
  assert.equal(result.details.rawError, quotaMessage)
})
