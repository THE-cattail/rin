...
LLMs have limited context windows. When conversations grow too long, rin uses compaction to summarize older content while preserving recent work. This page covers both auto-compaction and branch summarization.
...
Rin has two summarization mechanisms:
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.rin/settings.json` or `<project-dir>/.rin/settings.json`). This leaves room for the LLM's response.

You can also trigger manually with `/compact [instructions]`, where optional instructions focus the summary.

### How It Works

1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.rin/settings.json` or `<project-dir>/.rin/settings.json`) is reached
...
For split turns, rin generates two summaries and merges them:
...
When you use `/tree` to navigate to a different branch, rin offers to summarize the work you're leaving. This injects context from the left branch into the new branch.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, rin extracts file operations from:
...
Configure compaction in `~/.rin/settings.json` or `<project-dir>/.rin/settings.json`:
