...
```
~/.rin/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.rin/sessions/`.

Rin also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, rin uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@mariozechner/pi-coding-agent/dist/` and `node_modules/@mariozechner/rin-ai/dist/`.
...
### Base Message Types (from rin-ai)
...
### Extended Message Types (from rin-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if rin-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` command or `rin.setSessionName()` in extensions.
