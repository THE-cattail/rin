...
export function resolveMemoryRoot(rootOverride = ''): string {
  ...
  return path.join(resolveRinLayout().homeRoot, 'memory')
}
...
function renderMarkdownDoc(doc: MemoryDoc): string {
  ...
  const header = yaml.dump(fm, { lineWidth: 120 }).trimEnd()
...
function resolveMemoryIndexRoot(rootOverride = ''): string {
  ...
  return path.join(resolveRinLayout().dataDir, 'memory-index')
}
...
function assertResidentDoc(doc: MemoryDoc): void {
  ...
  if (!RESIDENT_SLOTS.includes(slot)) throw new Error(`resident_slot_required:${RESIDENT_SLOTS.join(',')}`)
...
export async function searchMemories(query: string, params: Record<string, any> = {}, rootOverride = '') {
  ...
  try {
    ...
    vectorStatus = { enabled: false, error: safeString(e && e.message ? e.message : e) }
  }
...
function compileFromDocs(docs: MemoryDoc[], params: Record<string, any> = {}, root = '') {
  ...
  const resident = docs
    .filter((doc) => doc.exposure === 'resident' && doc.canonical && RESIDENT_SLOTS.includes(doc.resident_slot))
    .sort((a, b) => RESIDENT_SLOTS.indexOf(a.resident_slot) - RESIDENT_SLOTS.indexOf(b.resident_slot))
...
export async function doctorMemory(rootOverride = '') {
  ...
  try {
    ...
    vector = { enabled: false, error: safeString(e && e.message ? e.message : e) }
  }
  return {
    root,
    resident_slots: RESIDENT_SLOTS,
